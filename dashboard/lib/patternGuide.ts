...
export const PATTERN_GUIDE: Record<PatternKey, PatternGuide> = {
  common_trend: {
    ...
    concept:
      ...
      "박스 천장(Darvas), 피벗(VCP). 서로 다른 방식으로 같은 사건을 확인하면 신뢰도가 올라간다는 '발상'이며,\n" +
      "아래 실측대로 그 발상은 아직 데이터로 확인되지 않았습니다.",
    period: "스윙 ~ 중기 (4주 ~ 6개월)",
    note:
      "⚠️ 겹침 이득 미확인. 실측(2025-04~2026-08) 결과 이 목록의 선정력은 유니버스 대비 " +
      "5일 −0.37% / 20일 −0.20% / 60일 −0.14%로 세 기간 모두 0 아래입니다. " +
      "'겹치면 신뢰도가 올라간다'를 뒷받침하는 수치가 없습니다. " +
      "겹침 보정 t는 −0.51 / −0.20 / −0.16으로 통계적으로는 노이즈와 구별되지 않습니다 — " +
      "해롭다고 입증된 것도 아니고, 이득이 있다고 볼 근거도 없다는 뜻입니다.\n" +
  ...
  common_accum: {
    ...
    period: "중기 ~ 장기 (2개월 ~ 1년+)",
    note:
      "구성이 1개라 이 목록은 Wyckoff 단독과 동일합니다 — 실측치도 Wyckoff 항목과 같습니다 " +
      "(선정력 5일 +0.20% / 20일 +0.16% / 60일 −0.78%, 모두 t<1로 노이즈 수준).\n" +
  ...
  common_all: {
    ...
    note:
      ...
      "⚠️ 점수 정렬에 정보가 있다는 근거가 없습니다. 화면에 뜨는 상위 20종목을 따로 재면 " +
      "게이트 통과 전체 대비 5일 +0.53%/+0.28%(개선), 20일 −0.73%/+0.02%(악화), " +
      "60일 +1.12%/+1.36%(악화)로 방향이 기간마다 뒤집힙니다. 상위권을 더 신뢰할 이유가 없습니다.",
  },
  canslim: {
    title: "CAN SLIM",
