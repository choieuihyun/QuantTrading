...
- 실행 시각 (2026-08-20 변경, 하루 3회)
  | KST | UTC 크론 | 목적 |
  |---|---|---|
  | 08:30 | `30 23 * * 0-4` | 장 시작 전 — 전날 완성봉 기준 |
  | 15:40 | `40 6 * * 1-5` | 정규장 마감 직후 |
  | 20:00 | `0 11 * * 1-5` | 데이터 확정 + 저녁 DART 공시 |

  - **15:40은 일봉이 확정되기 전일 수 있다.** 종전에 18:00을 쓴 이유가 그것이었다.
    같은 날 20:00 실행이 같은 문서를 덮어쓰므로 최종값은 확정본이 된다.
  - **09시대는 쓰지 않는다.** 장중이라 당일 봉이 미완성이고, `vol_ratio`가
    (몇 분 거래량 ÷ 20일 평균 일거래량)이 되어 0에 가까워진다.
    `vol_ratio >= 2`를 요구하는 CAN SLIM의 S와 Darvas가 전부 탈락한다.
    KR 설정의 `drop_partial_bar: False`가 "장 마감 후 실행" 전제 위에 있다.
  - 월 20:00(UTC 월 11:00)과 화 08:30(UTC 월 23:30)은 UTC 날짜가 같아
    `screener_results` 문서를 공유한다. 둘 다 월요일 종가 기준이라 손실은 없다.
...
표준 공식과 대조해 실측했다. "내부 일관성"(라이브=재현)과 "정의 일치"는 다른 문제다 —
공식이 틀려도 두 경로가 같은 코드를 쓰면 일관성 검사는 통과한다.

| 지표 | 이전 | 표준 | 실측 차이 | 조치 |
|------|------|------|-----------|------|
| RSI | 단순이동평균 (Cutler) | Wilder 평활 | 평균 4.06p / 최대 15.5p<br>게이트 판정 60종목 중 15종목 반전 | 수정 |
| ATR | 단순이동평균 | Wilder 평활 | 평균 4.5% / 최대 15.2%<br>손절가 주가 대비 최대 1.72%p | 수정 |
| 볼린저 σ | 표본(ddof=1) | 모집단(ddof=0) | `bb_squeeze` 판정 반전 0/60건 | 유지 |
| MACD | `ewm(span, adjust=False)` | 동일 | — | 유지 |
| 이동평균·OBV | 단순/표준 정의 | 동일 | — | 유지 |

볼린저를 고치지 않은 이유 — `bb_squeeze`가 자기 60일 분위수와 비교하는 구조라
σ에 곱해지는 상수(√(20/19) ≈ 1.026)가 분자·분모에서 상쇄된다.
